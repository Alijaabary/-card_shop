...
from .forms import ProductForm, ProductUpdateForm, ProductCardShoptModelFormSet
...
def product_manage_detail_view(request, handle=None):
    ...
    attachments = prouductcardshop.objects.filter(product=obj)
    is_manager = False
    if request.user.is_authenticated:
    ...
    formset = ProductCardShoptModelFormSet(request.POST or None, 
...
def product_detail_view(request, handle=None):
    obj = get_object_or_404(Product, handle=handle)
    attachments = prouductcardshop.objects.filter(product=obj)
    # attachments = obj.prctattachment_set.all()
    is_owner = False
    if request.user.is_authenticated:
...
def product_cardshop_download_view(request, handle=None, pk=None):
    attachment = get_object_or_404(prouductcardshop, product__handle=handle, pk=pk)
